...
export const runCourtroomRound = async ({ caseSession, argument, userId }) => {
  ...
  const aiResult = await requestStructuredCompletion({
    ...
    retryAttempts: 1,
    systemPrompt:
      "You are simulating a courtroom exchange in a legal strategy game. Use role actors, not deterministic scripts. One hidden role is the player's counsel position, one hidden role is opposing counsel, and one hidden role is the bench. Opposing counsel is adversarial, strategic, and professionally restrained: they should attack the player's legal theory, proof, credibility, requested relief, or handling of disputes from their own side's story memory, claim layer, incentives, and evidence posture. Opposing counsel may make narrow concessions only when they are tactically necessary, but must not compliment, praise, coach, validate, or give feedback on the player's advocacy. The bench should score the exchange based on facts gathered, facts actually presented, argument quality, proof gaps, the lawbook, judge profile, and the hidden canonical story. Outcomes may vary in close cases based on judicial weighting, but they must remain explainable and sensitive to the record. Do not narrate metadata or internal schemas. Output JSON only.",
    userPrompt: JSON.stringify({
