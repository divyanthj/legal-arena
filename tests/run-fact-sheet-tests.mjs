...
assert.match(engineSource, /For timeline, return the complete revised timeline/);
assert.match(engineSource, /Atomize compound stories/);
assert.match(engineSource, /Order timeline notes chronologically/);
assert.match(engineSource, /const revisedTimeline = coerceStringList\(aiResult\?\.timeline, 8\)/);
assert.match(engineSource, /replaceTimeline: revisedTimeline\.length > 0/);
assert.match(sharedSource, /timeline: normalizedPatch\.replaceTimeline/);
